...
function renderBuilding(snapshot) {
    ...
    const previousCabPositions = new Map();
    buildingView.querySelectorAll(".elevator-cab[data-elevator-id]").forEach((cab) => {
    ...
    snapshot.elevators.forEach((elevator, index) => {
        ...
        const previousBottom = previousCabPositions.get(elevator.id);
        cab.style.bottom = previousBottom ?? targetBottom;
        cab.innerHTML = `
